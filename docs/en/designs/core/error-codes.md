...
> `LLM_CALL_FAILED`) are **string-based message patterns** extracted from
> source code — they are informal convenience labels, not a formal error
> taxonomy. The **authoritative structured error types** are the enums defined
> in [`packages/shared/core/src/errors.rs`](../packages/shared/core/src/errors.rs):
> `AgentErrorCode` (line 8), `StructuredAgentError`, `CoreError`, `CredentialError`,
> `PromptLoadError`, `SoulLoadError`, and their variants. When integrating or
> reporting errors programmatically, use those enums, not the string patterns
> listed here.

This document catalogs the error patterns used across Entelecheia's Rust codebase.
Structured error codes are work-in-progress; most errors currently use `anyhow`/`thiserror`
...
> **Note**: This is a best-effort catalog. Entelecheia is migrating toward
> structured error types with unique codes. Contributions to expand this
