...
**是什么**：对技能输出与执行轨迹进行基於规则的检查。

**何时**：每个技能，无例外。

**怎么做**：纯 Rust 逻辑，无 LLM 呼叫。在现有
`validate_report_capture()` 的基础上扩展，加入额外的启发式规则：

- 输出长度合理性（过短 / 可疑地过长）
- 工具呼叫模式异常（呼叫了技能预期领域之外的工具）
- 执行时间离群值（技能耗时是其历史平均值的 10 倍以上）
- 工具呼叫内部的错误率（超过 50% 的工具呼叫失败）
- 输出中的循环參照检测

**成本**：接近零（微秒级 CPU）。

**裁定**：可发出 `Accept`（通过）或 `NeedsTier1`（升级到语义層）。

### Tier 1：语义反思（1 次 LLM 呼叫，位于决策点）

**是什么**：基於 LLM 的评估：“这个输出是否达成了技能所声明的
目标？推理链是否内部自洽？”
...
**怎么做**：呼叫 OreXis agent，注入一段反思专用的技能提示。该提示
包含：

- 技能声明的目标
- 技能的输出
- 执行轨迹摘要（做了哪些工具呼叫、各自的结果）
- 链上下文（之前是什么、之后是什么）

**成本**：1 次 LLM 呼叫（约 500-2000 输出 token）。
...
- 操作者設定的周期性（例如每 N 条链一次）
- YOLO Strategic 等级

**怎么做**：呼叫带 `deep_critique` 技能的 OreXis agent。审查：

- 完整的链轨迹（所有技能、所有输出、所有工具呼叫）
- 原始目标 vs. 实际达成的结果
- 来自经验库的历史经验（用于模式匹配）

**成本**：1-2 次 LLM 呼叫（约 1000-5000 输出 token）。
