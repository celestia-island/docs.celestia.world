...
如何将 evernight 連線到每个受支援的協定、使用什么服务端软件，以及如何端到端地验证連線是否正常工作。
...
### 服务端
- **裝置**：任意 Modbus RTU 从站（PLC、传感器、变频器）
- **无伺服器测试**：`socat PTY,raw,echo=0,link=/tmp/vcom_a PTY,raw,echo=0,link=/tmp/vcom_b &`
...
### 服务端
- **裝置**：S7-1200/1500/300/400 PLC
- **前置条件**：TIA Portal → 启用“允许 PUT/GET”+ 禁用“最佳化块访问”
...
### 服务端
- **裝置**：MELSEC FX/Q/L/iQ-R PLC
- **无硬件测试**：`tests/mc_test_server.rs`（进程内的 mock MC 伺服器）
...
### 服务端
- **裝置**：Allen-Bradley CompactLogix / ControlLogix
...
### 服务端
- **裝置/软件**：任意 OPC UA 伺服器（KEPServerEX、Ignition、CODESYS 等）
- **自托管**：evernight 本身就可以充当 OPC UA 伺服器：
...
### RDP 当前进展
- ✅ 传输層：X.224 + TLS 升级（已对照真实 xrdp 验证）
...
#### Tier 3：基於现有 API 的 Web 前端（最大投入，生产级）

Evernight 已有 `api-serve --transport ws`（基於 WebSocket 的 JSON-RPC）。
一个 Web 前端（shittim-chest / Tauri）連線到该 API 并：

- 在 `<canvas>` 上渲染 framebuffer
- 通过 JSON-RPC 发送输入事件
- 这是生产路径——Tier 1+2 是为开发/测试用的

这是前端工作（Vue/React/Tauri），而非 evernight 库程式碼。
