...
>
> 不预设"弱/强"分类。环境变量传了哪个 provider 的 key,就创建哪个模型的编码套餐配置并测试。一个 key 都没有 → 直接报错退出。
...
- `_shared_llm_provider::ProviderRegistry`:全局注册,5 种协议(OpenAI Chat / Responses / Anthropic v1/v2 / Gemini)
...
Profile 在运行时从 env vars 动态构建,不预存配置文件。
...
### 3.1 Record/Replay 协议

Mock 服务端兼容 OpenAI Chat Completions API(`/v1/chat/completions`),支持两种工作模式:
...
**Strict 模式**(CI 默认开启):任何未匹配的请求立即报错,不 fallback 到真实 API。确保 fixture 完整。
...
| 方案 | 优点 | 缺点 |
|------|------|------|
| **AIMock**(CopilotKit) | 成熟、支持 streaming/tool-calls/MCP、Docker 镜像 | 外部依赖 |
| **自建简易服务器** | 完全可控、零外部依赖 | 需自己处理 streaming/edge case |
| **VCR.py / wiremock** | 语言生态成熟 | 非 LLM 专用,需适配 |

**推荐**:先用自建简易服务器(一个 axum/actix 路由,匹配 + 返回 JSON),后续如需 streaming 再迁移到 AIMock。
...
- 命令执行工具 → 映射到容器内 bash
- 搜索工具 → `rg`/`grep`(容器内预装)
- **禁用**与该任务无关的 agent(PoleMos/工业协议等),减少上下文噪声
...
输出:每个实例 resolved/unresolved,汇总 resolution rate。

## 5. 实验矩阵

### 5.1 核心对比

固定数据集(SWE-bench Verified),变化两个维度:
...
- [ ] 实现 OpenAI 兼容的 record/replay 服务器
- [ ] 请求哈希匹配 + strict/lenient 模式
- [ ] Fixture 存储结构
- [ ] `ENTELECHEIA_LLM_BASE_URL` 环境变量切换

### Phase 2:SWE-bench 适配器(2-3 天)
- [ ] JSONL 任务加载器
- [ ] Docker 容器编排(复用 SWE-bench 镜像)
- [ ] Agent Runtime 注入容器
- [ ] Patch 提取 + JSONL 输出
- [ ] 接入原生 harness 评分
...
| `KaLos` 文件 | 映射到容器内文件系统 |
| `OreXis` 安全 | benchmark 模式下放宽安全策略(允许任意代码执行) |

## 8. 注意事项

- **成本控制**:SWE-bench Verified 全量 500 题 × 4 模型 × 2 配置 = 4000 次运行。按平均 50 步/题、每步 ~2K tokens 估算,约 400M tokens。需设置 `--max_cost` 上限。
- **容器资源**:每个 SWE-bench 实例需要独立 Docker 容器,建议 ≥120GB 磁盘、≥32GB RAM。
